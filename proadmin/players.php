...
<br>
<h1 class="main-heading">Players</h1>
        ...
        $uid = $row['uid'];
     	...
     	echo "<tr class=\"hover\">
            ...
            <td style=\"font-size:15px;\">{$name}</td>
              ...
              <td style=\"font-size:15px;\"><a id=\"link\" href=\"playerinfo.php?uid={$uid}\">Info</a></td>
     	      <td style=\"font-size:15px;\"><a id=\"link\" href=\"deleteplayer.php?uid={$uid}\" onclick=\"return confirm('Are you sure?')\">Delete</a></td>
              <td style=\"font-size:15px;\"><a id=\"link\" href=\"disqualifyplayer.php?uid={$uid}\" onclick=\"return confirm('Are you sure?')\">Disqualify</a></td>
     	      </tr>";
     $rank++;
     }
     ?>
...
<br>
<hr>
<h1 class="main-heading">Disqualified Players</h1>
<table class="leaderboard" cellspacing="0">
        <th>Serial</th>
        <th>Name</th>
        <th>Username</th>
    ...
    $rank = 1;
    $completed = 20; //Set Last Level
     $result = mysql_query("SELECT * FROM players WHERE status=0 AND usertype=1 ORDER by level AND time DESC");
     while ($row = mysql_fetch_assoc($result)) {
        $name = $row['name'];
        $player = $row['username'];
        $level = $row['level'];
        ...
        echo "<tr class=\"hover\">
              ...
              <td style=\"font-size:15px;\"><a id=\"link\" href=\"playerinfo.php?uid={$uid}\">Info</a></td>
              <td style=\"font-size:15px;\"><a id=\"link\" href=\"deleteplayer.php?uid={$uid}\" onclick=\"return confirm('Are you sure?')\">Delete</a></td>
              <td style=\"font-size:15px;\"><a id=\"link\" href=\"qualifyplayer.php?uid={$uid}\" onclick=\"return confirm('Are you sure?')\">Qualify</a></td>
              </tr>";
     $rank++;
     }
     ?>
